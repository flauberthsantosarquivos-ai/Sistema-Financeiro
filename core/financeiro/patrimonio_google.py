...
def atualizar_saldo_pagbank_por_alimentacao(
    ...
    """
    Atualiza exclusivamente PAGBANK / FLV|RESTAURANTE|SUPERMERCADO
    a partir do saldo final lido no extrato de Alimentação.

    Preserva todos os demais ativos e, quando a linha já existe,
    preserva VALOR_INICIO e apenas substitui VALOR_FIM.
    """
    ano = str(ano or "").strip()
    mes = normalizar_upper(mes)
    subdivisao = normalizar_upper(subdivisao)
    ...
    if subdivisao not in SUBDIVISOES_PADRAO.get("PAGBANK", []):
        raise ValueError("Subdivisão PagBank inválida para atualização patrimonial.")

    aba = obter_ou_criar_aba_patrimonio()
    valores = aba.get_all_values()
    agora = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    observacao = "Saldo atualizado automaticamente pelo extrato PagBank/Alimentação."
    ...
    for numero_linha, linha in enumerate(valores[1:], start=2):
        ...
        if (
            ...
            and normalizar_upper(item.get("MES")) == mes
            and normalizar_upper(item.get("ATIVO")) == "PAGBANK"
            and normalizar_upper(item.get("SUBDIVISAO")) == subdivisao
        ):
    ...
    if linha_encontrada:
        aba.update(
            f"H{linha_encontrada}:L{linha_encontrada}",
            [[saldo_final_num, diferenca, percentual, observacao, agora]],
